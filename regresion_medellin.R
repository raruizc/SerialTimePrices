medellin_pib <- read_excel("medellin_pib.xlsx")

modelo_medellin <- lm(formula = Precio ~ PIB,
                         data = medellin_pib)

summary(modelo_medellin)

ggplotly(
  ggplot(modelo_medellin, aes(x = PIB, y = Precio)) +
    geom_point(color = "#39568CFF") +
    geom_smooth(aes(color = "Fitted Values"),
                method = "lm", 
                level = 0.95) +
    labs(x = "PIB-Antioquia (Miles de Millones COP)",
         y = "Precio (COP)") +
...
historico_medellin <- read_excel("historico_medellin.xlsx")
historico_medellin <- historico_medellin %>% mutate(Data = as.Date(Data))

#Transformando a base de dados em um objeto de classe ts
precios_ts_med <- ts(data = historico_medellin[, 2],
                 start = c(2015, 1),
                 end = c(2021, 12),
                 frequency = 12)
...
decpib <- decompose(x = precios_ts_med,
                    type = "additive")



#Transformando o objeto decpib num data frame
decpib_df <- data.frame(tempo = historico_medellin$Data,
                        serie = unlist(decpib$x),
                        tendencia = unlist(decpib$trend),
                        sazonalidade = unlist(decpib$seasonal),
                        dessazonalizada = precios_ts_med - decpib$seasonal,
...
precio_entreno=window(precios_ts_med, start=c(2015,1), end=c(2020,12))
precio_teste=window(precios_ts_med, start=c(2021,1), end=c(2021,12))
length(precio_teste)
...
testeprecio
summary(testeprecio)
adf.test(precios_ts_med, alternative = "stationary") ##Test indica no es estacional

#Con una diferencia 
#### Debe ser estacionaria
seriedifprecio=diff(precios_ts_med)
seriedifprecio
