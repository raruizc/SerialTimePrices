bucaramanga_pib <- read_excel("bucaramanga_pib.xlsx")
modelo_bucaramanga <- lm(formula = Precio ~ PIB,
                    data = bucaramanga_pib)

summary(modelo_bucaramanga)

ggplotly(
  ggplot(modelo_bucaramanga, aes(x = PIB, y = Precio)) +
    geom_point(color = "#39568CFF") +
    geom_smooth(aes(color = "Fitted Values"),
                method = "lm", 
                level = 0.95) +
    labs(x = "PIB-Santander (Miles de Millones COP)",
         y = "Precio (COP)") +
...
historico_bucaramanga <- read_excel("historico_bucaramanga.xlsx")
historico_bucaramanga <- historico_bucaramanga %>% mutate(Data = as.Date(Data))

#Transformando a base de dados em um objeto de classe ts
precios_ts_buc <- ts(data = historico_bucaramanga[, 2],
                 start = c(2015, 1),
                 end = c(2021, 12),
                 frequency = 12)
...
decpib <- decompose(x = precios_ts_buc,
                    type = "additive")



#Transformando o objeto decpib num data frame
decpib_df <- data.frame(tempo = historico_bucaramanga$Data,
                        serie = unlist(decpib$x),
                        tendencia = unlist(decpib$trend),
                        sazonalidade = unlist(decpib$seasonal),
                        dessazonalizada = precios_ts_buc - decpib$seasonal,
...
decpib_df %>%
  ...
  geom_line(aes(x = tempo, y = sazonalidade, color = "Estacionalidad"), size = 1.2) +
...
precio_entreno=window(precios_ts_buc, start=c(2015,1), end=c(2020,12))
precio_teste=window(precios_ts_buc, start=c(2021,1), end=c(2021,12))
length(precio_teste)
...
seriedifprecio=diff(precios_ts_buc)
seriedifprecio
...
ggplotly(
  autoplot(precio_entreno)+
    ...
    autolayer(prevpreciobog$mean, serie="Pronóstico")+
    labs(x = "Tiempo",
         y = "Precio (COP)")+
